shinyServer(function(input, output, session){
   ...
   # analysis
   variables <- gather(zip.data, var, value, c(5:8, 10))

   output$plot.matrix <- renderPlot({
      ggplot(variables, aes(x = value, y = housing.cost, color = housing.cost),
   ...
   output$hover.info <- renderUI({
      ...
      wellPanel(
         style = style,
         p(tags$b("block group:"), point.1$GEOID, br(),
                       tags$b("city:"), point.1$city, br(),
                       tags$b("zip:"), point.1$zip, br(),
                       tags$b("monthly housing cost: $"), point.1$housing.cost, br(),
                       tags$b("factor:"), point.1$var, br(),
                       tags$b("value:"), point.1$value, br())
      )
   })
