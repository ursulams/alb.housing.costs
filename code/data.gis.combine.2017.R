...
smart <- smart %>% 
  dplyr::select(GEOID = GEOID10, cbsa.total.employ = CBSA_EMP, cbsa.total.workers = CBSA_WRK, 
        ...
        two.plus.auto = AUTOOWN2P, num.workers.home = WORKERS, low.wage.home = R_LOWWAGEW, 
        med.wage.home = R_MEDWAGEW, hi.wage.home = R_HIWAGEWK, 
        ...
        job.type.mix = D2B_E8MIX, low.wage.work = E_LOWWAGEW,
        med.wage.work = E_MEDWAGEW, hi.wage.work = E_HIWAGEWK,
...
# impute missing observations via mice regression
imp <- mice(all.data, maxit = 0)
method <- imp$method
method[c("GEOID")]=""
pred <- imp$predictorMatrix
set.seed(123)
imputed.all <- mice(all.data, method = method, predictorMatrix = pred)

all.data <- complete(imputed.all)
